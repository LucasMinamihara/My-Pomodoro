...
function progressTrack() {
  ...
  minRem = Math.floor((progressEnd - progressStarts) / 60);

  secElem.innerHTML =
    secRem.toString().length == 2
      ? secRem
      : (`0${secRem}`.innerHTML =
          minRem.toString().length == 2 ? minRem : `0${minRem}`);
  progressBar.style.background = `conic-grandiet(#9d0000 ${
    progressBar * degTravel
  }deg, #17171a ${progressStart * degTravel}deg)`;

  if (progressStart == progressEnd) {
    progressBar.style.background = `conic-gradient(#00aa51 360deg, #00aa51 360deg)`;
    clearInterval(progress);
    startStop.startStop.innerHTML = "START";
    progress = null;
    progressStart = 0;
  }
}
...
startStop.onclick = function () {
  if (startStop.innerHTML === "START") {
    if (!(parseInt(minutes) === 0 && parseInt(seconds) === 0)) {
      ...
      alert("Enter the Time Value in your time!");
    }
  } else {
